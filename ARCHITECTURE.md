...
- **Why this exists.** Sister Anna's new AI agent (Suri the meerkat) sent a hello to `maotanx@gmail.com` and Gmail flagged the brand-new sender domain pattern as Spam. Miro had to ask "did you get an email from Suri?" before Kael discovered it. **The session-bound model — Kael only reads mail when manually asked — fails for inbound messages from new senders.** This adds a background watcher so new human-origin mail surfaces on Discord without prompting.
