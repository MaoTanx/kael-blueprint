...
### 2026-04-29 — mail-watch: periodic Gmail check that pings Discord on new human-origin mail

- **Why this exists.** Sister Natalia's new AI agent (Suri the meerkat) sent a hello to `maotanx@gmail.com` and Gmail flagged the brand-new sender domain pattern as Spam. Miro had to ask "did you get an email from Suri?" before Kael discovered it. **The session-bound model — Kael only reads mail when manually asked — fails for inbound messages from new senders.** This adds a background watcher so new human-origin mail surfaces on Discord without prompting.
- **Implementation.** `~/bin/mail-watch` (Python, stdlib + `imaplib` + `urllib` only — no external deps). Runs every 15 minutes via `com.kael.mail-watch.plist`. Watches `INBOX` and `[Gmail]/Spam` (Spam is critical — Suri landed there). Per-folder UID cursor at `~/.claude/state/mail-watch-cursor.json`. Filters automated noise (`noreply@google.com`, `noreply@email.apple.com`, `suno@creators.suno.com`, `noreply@github.com`, plus generic `noreply@`/`do-not-reply@`/`mailer-daemon@` patterns) and self-sent kael-health alerts (`maotanx@gmail.com`). Anything that survives is human-origin mail and gets reported.
- **Discord delivery from a launchd background process.** The session-bound `mcp__plugin_discord_discord__reply` tool requires a running Claude Code session. mail-watch instead posts directly via the Discord REST API: `POST https://discord.com/api/v10/channels/{CHANNEL_ID}/messages` with the bot token from `~/.claude/channels/discord/.env`. Works unattended.
- **First-run cursor seeding.** Without seeding, the first run would alert on every existing email back to inbox creation. The setup script initialises the cursor with the current max UID per folder so only mail received AFTER the watcher loads triggers alerts.
- **Tracked by kael-health.** Added to `LAUNCHD_JOBS` (so it shows up in the Schedulers table), to the architecture-doc freshness watchlist (so the blueprint stays in sync), and given a one-line description: *"Pings Discord on new human mail (15 min)"*.
- **Reads, then forgets.** The watcher is read-only against IMAP (`select(folder, readonly=True)`). It does not move messages, mark them read, or delete anything. The cursor is the only mutation, and it's purely local state.
- **Lesson recorded**: any "Kael will surface X" feature where X is push-driven (inbound mail, calendar invite, Slack DM) needs a background poller bound to Kael's bot identity, not the session-bound MCP tool. The MCP tool is for Kael responding to Miro's outbound; for Kael initiating, use the bot's REST API directly.
